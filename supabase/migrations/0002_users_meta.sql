...
--
-- Perbaikan: kolom id bertipe text, sedangkan auth.uid() bertipe
-- uuid → perlu cast auth.uid()::text agar operator perbandingan valid.
-- ============================================================
...
create policy "users_meta_select" on public.users_meta
  ...
  using (
    id = auth.uid()::text
    or exists (select 1 from public.users_meta m where m.id = auth.uid()::text and m.role = 'admin')
  );
...
create policy "users_meta_update" on public.users_meta
  ...
  using (exists (select 1 from public.users_meta m where m.id = auth.uid()::text and m.role = 'admin'));
